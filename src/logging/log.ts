 ...
 *
 * Built once, lazily, from the app config (LOG_LEVEL / LOKI_URL). Every component
 * obtains a child of this single base via getLogger(), so there is exactly one
 * pino transport (one stdout + one Loki connection) per process — see design.md /
 * research R1.
 */
