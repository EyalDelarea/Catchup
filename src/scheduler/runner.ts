 ...
 *
 * Startup catch-up (T025): on start, for each slot, if dueCatchup() is true,
...
export function startScheduler(opts: StartSchedulerOpts): SchedulerHandle {
  ...
  /**
   * Startup catch-up: check every slot; if dueCatchup() is true, run once.
   *
   ...
   * check the overall dueCatchup across all times.
   *
   * Simpler and correct: use a single catch-up key "digest@catchup" that
  ...
  async function runStartupCatchup(): Promise<void> {
    ...
    try {
      // Find the most recent slot that has run (to drive overall dueCatchup).
      ...
      // If no slot has ever run, latestLastRun is null → dueCatchup = true.
