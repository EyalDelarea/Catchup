...
describe("normalizeEngineConfig", () => {
  it("returns defaults for a missing / non-object blob", () => {
    expect(normalizeEngineConfig(undefined)).toEqual(engineDefaults());
    expect(normalizeEngineConfig(null)).toEqual(engineDefaults());
    expect(normalizeEngineConfig("nope")).toEqual(engineDefaults());
  });

  it("fills only the absent fields, keeping provided ones", () => {
    const cfg = normalizeEngineConfig({ on: true, kinds: { task: false } });
    expect(cfg.on).toBe(true);
    expect(cfg.kinds).toEqual({ task: false, meeting: true, followup: true, recap: true });
    expect(cfg.proact).toBe("מאוזן");
  });

  it("preserves unknown fields so a UI round-trip never drops engine state", () => {
    const cfg = normalizeEngineConfig({ on: true, learned: { weights: [1, 2] } });
    expect(cfg.learned).toEqual({ weights: [1, 2] });
  });

  it("ignores non-boolean kind values", () => {
    const cfg = normalizeEngineConfig({ kinds: { task: "yes" } });
    expect(cfg.kinds.task).toBe(true);
  });
});
