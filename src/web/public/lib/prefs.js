...
// layer so they can be unit-tested. Two concerns:
//   • the digest-times picker — CSV ⇄ selected HH:MM chips
//   • the experimental engine config — normalize the opaque blob into a shape
//     the Settings UI can render (the S6 engine owns its real meaning)
...
/** The proactiveness levels offered by the engine section (gentle → eager). */
export const PROACT_LEVELS = ["עדין", "מאוזן", "יוזם"];

/** The suggestion kinds the engine can surface. */
...
/** Default engine config — off, all kinds on, balanced proactiveness. */
...
/**
 * Coerce the opaque `engineConfig` blob into the shape the Settings UI renders,
 * filling gaps from {@link engineDefaults}. Unknown extra fields are preserved
 * so a round-trip through the UI never drops engine state it doesn't surface.
 * @param {unknown} cfg
 * @returns {{on: boolean, kinds: Record<string, boolean>, proact: string} & Record<string, unknown>}
 */
export function normalizeEngineConfig(cfg) {
  const d = engineDefaults();
  if (!cfg || typeof cfg !== "object") return d;
  const src = /** @type {Record<string, unknown>} */ (cfg);
  const kinds = { ...d.kinds };
  if (src["kinds"] && typeof src["kinds"] === "object") {
    for (const [k, v] of Object.entries(src["kinds"])) {
      if (typeof v === "boolean") kinds[k] = v;
    }
  }
  return {
    ...src,
    on: typeof src["on"] === "boolean" ? src["on"] : d.on,
    kinds,
    proact: typeof src["proact"] === "string" ? src["proact"] : d.proact,
  };
}
