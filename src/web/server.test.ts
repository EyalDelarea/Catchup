...
describe("web server", () => {
  ...
  it("catchup first-run: streams status/token/done, persists watermark and summary", async () => {
    const name = `WEB-catchup-firstrun-${randomUUID()}`;
  ...
  it("catchup cache-hit: second request with no new messages emits cached, no new writes", async () => {
    ...
    const name = `WEB-catchup-cachehit-${randomUUID()}`;
    const g = await upsertGroup(pool, { name, source: "import" });
...
describe("handleSummarize with backfill deps", () => {
  ...
  it("existing under-window catchup backfill still fires when held<25 (no since)", async () => {
...
describe("handleSummarize — client disconnect aborts summarizer, no commit", () => {
  ...
  it("catchup path: disconnect mid-stream → signal forwarded and no summary or watermark committed", async () => {
    const name = `DC-catchup-${randomUUID()}`;
    const g = await upsertGroup(pool, { name, source: "import" });
    await seedText(g, "hello catchup disconnect", `dc-catchup-${randomUUID()}`);
    ...
    try {
      await new Promise<void>((resolve) => {
        const req = http.get(
          `http://localhost:${port}/api/summarize?group=${encodeURIComponent(name)}&mode=catchup`,
          (res) => {
            res.once("data", () => {
              req.destroy();
              resolve();
