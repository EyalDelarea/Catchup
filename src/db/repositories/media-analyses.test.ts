...
describe("media-analyses", () => {
  ...
  describe("selectPendingVisualMedia", () => {
    it("returns message IDs for present visual media without a completed analysis, newest-first", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending", source: "import" });

      const older = await seedMediaMessage(groupId, {
        dedupeKey: "pending-old-1",
      ...
      const newer = await seedMediaMessage(groupId, {
        dedupeKey: "pending-new-1",
        mediaFilename: "new.png",
        mediaPath: "/data/new.png",
        mediaStatus: "present",
        sentAt: new Date("2026-01-02T08:00:00.000Z"),
      });

      const result = await selectPendingVisualMedia(pool, 10);
      expect(result).toContain(older);
      expect(result).toContain(newer);
      // Newer comes first (newest-first ordering)
    ...
    it("excludes messages that already have a completed analysis", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending-excl", source: "import" });

      const analyzed = await seedMediaMessage(groupId, {
        dedupeKey: "pending-analyzed-1",
        mediaFilename: "analyzed.jpg",
        mediaPath: "/data/analyzed.jpg",
        mediaStatus: "present",
      });
      const pending = await seedMediaMessage(groupId, {
        dedupeKey: "pending-pending-1",
        mediaFilename: "pending.jpg",
        mediaPath: "/data/pending.jpg",
        mediaStatus: "present",
      });
      ...
      const result = await selectPendingVisualMedia(pool, 1000);
      expect(result).not.toContain(analyzed);
      expect(result).toContain(pending);
    });

    it("excludes messages with a failed analysis (selectPendingVisualMedia uses any-row check)", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending-failed", source: "import" });

      const failedMsg = await seedMediaMessage(groupId, {
        dedupeKey: "pending-failed-1",
        mediaFilename: "failed.jpg",
        mediaPath: "/data/failed.jpg",
        mediaStatus: "present",
      });

      // selectPendingVisualMedia uses LEFT JOIN / a.id IS NULL (any row), so a failed row
      // still excludes the message. Use selectVisualMediaNeedingAnalysis to include failed rows.
      await insertMediaAnalysis(pool, {
        messageId: failedMsg,
        kind: "image",
        description: null,
      ...
      const result = await selectPendingVisualMedia(pool, 10);
      expect(result).not.toContain(failedMsg);
    });

    it("excludes missing media", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending-missing", source: "import" });

      const missingMsg = await seedMediaMessage(groupId, {
        dedupeKey: "pending-missing-1",
      ...
      const result = await selectPendingVisualMedia(pool, 10);
      expect(result).not.toContain(missingMsg);
    });

    it("excludes audio media", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending-audio", source: "import" });

      const audioMsg = await seedMediaMessage(groupId, {
        dedupeKey: "pending-audio-1",
        mediaFilename: "voice.opus",
        mediaPath: "/data/voice.opus",
        mediaStatus: "present",
      });
      ...
      const result = await selectPendingVisualMedia(pool, 10);
      expect(result).not.toContain(audioMsg);
    });

    it("respects the limit parameter", async () => {
      const groupId = await upsertGroup(pool, { name: "MA-pending-limit", source: "import" });

      for (let i = 0; i < 5; i++) {
        await seedMediaMessage(groupId, {
          dedupeKey: `pending-lim-${i}`,
          mediaFilename: `img${i}.jpg`,
          mediaPath: `/data/img${i}.jpg`,
          mediaStatus: "present",
          sentAt: new Date(`2026-02-0${i + 1}T08:00:00.000Z`),
        });
      }

      const result = await selectPendingVisualMedia(pool, 3);
