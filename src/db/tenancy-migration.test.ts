...
describe("database roles", () => {
  it("catchup_app exists, can login, is not superuser, does not bypass RLS", async () => {
    const { rows } = await pool.query<{
      rolcanlogin: boolean;
      rolsuper: boolean;
      rolbypassrls: boolean;
    }>(`SELECT rolcanlogin, rolsuper, rolbypassrls FROM pg_roles WHERE rolname = 'catchup_app'`);
    expect(rows).toHaveLength(1);
    expect(rows[0]).toMatchObject({ rolcanlogin: true, rolsuper: false, rolbypassrls: false });
  });

  it("catchup_operator exists and bypasses RLS", async () => {
    const { rows } = await pool.query<{ rolbypassrls: boolean; rolsuper: boolean }>(
      `SELECT rolbypassrls, rolsuper FROM pg_roles WHERE rolname = 'catchup_operator'`,
    );
    expect(rows).toHaveLength(1);
